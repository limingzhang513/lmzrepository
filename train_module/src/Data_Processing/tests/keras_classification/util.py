...
# from tests.keras_classification import config
from . import config
...
def save_history(history, prefix):
    ...
    config.img_path = os.path.join(config.plots_dir, '{}-%s.jpg'.format(prefix))
    ...
    plt.savefig(config.img_path % 'accuracy')
    plt.close()
    ...
    plt.savefig(config.img_path % 'loss')
    plt.close()
...
def get_model_class_instance(*args, **kwargs):
    module = importlib.import_module("keras_classification.models.{}".format(config.model))
    return module.inst_class(*args, **kwargs)


def get_model_from_h5file(*args, **kwargs):
    module = importlib.import_module("keras_classification.models.load_model")
    return module.inst_class(*args, **kwargs)
